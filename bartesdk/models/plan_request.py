...
class PlanRequest(BaseModel):
    ...
    values: Optional[List[BasicValueRequest]] = None
    accept_payment_methods: Optional[List[StrictStr]] = Field(default=None, alias="acceptPaymentMethods")
    __properties: ClassVar[List[str]] = ["title", "description", "active", "bullets", "values", "acceptPaymentMethods"]

    @field_validator('accept_payment_methods')
    def accept_payment_methods_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        for i in value:
            if i not in set(['CREDIT_CARD', 'BANK_SLIP', 'PIX']):
                raise ValueError("each list item must be one of ('CREDIT_CARD', 'BANK_SLIP', 'PIX')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # override the default output from pydantic by calling `to_dict()` of each item in bullets (list)
        _items = []
        if self.bullets:
            for _item in self.bullets:
                if _item:
                    _items.append(_item.to_dict())
            _dict['bullets'] = _items
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "bullets": [BulletRequest.from_dict(_item) for _item in obj["bullets"]] if obj.get("bullets") is not None else None,
            "values": [BasicValueRequest.from_dict(_item) for _item in obj["values"]] if obj.get("values") is not None else None,
            "acceptPaymentMethods": obj.get("acceptPaymentMethods")
        })
        return _obj
